import sys
# /Data_all/GenomicDatabases/Mouse/Ensembl/gene_name.txt
...
for i in lines1:
    xx=i.strip().split('\t')
    dict1[xx[0]]=xx[1]
# analysis//3.GenesExpress/1.Gene_id
gene_id_dir=sys.argv[2]
# analysis//3.GenesExpress/2.Gene_name_part
outdir=sys.argv[3]
...
out2.write(lines2[0])
# 如果 gene_id 有 gene_name 就在后面附上没有就过滤
for i in lines2[1:]:
    xx=i.split('\t',1)
    if xx[0] in dict1:
