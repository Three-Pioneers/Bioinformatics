...
function initSidebar(sidebarQuery, contentQuery) {
    ...
    function updateSidebar(){
            ...
            // var title = document.querySelectorAll('article');
            for(i=0; i<allHeaders.length; i++){
                var link = allHeaders[i]
                if($(link).offset().top <= wst+15){
                    setActive(link.id, sidebar)
                }
            }
    };
...
function makeLink(h, tag, className) {
    ...
    allId=[].slice.call(document.querySelectorAll('[id]')).map(function(ele){return ele.id});
    if (!h.id) h.id = IdEscape(text)
    while (allId.indexOf(h.id)!=-1){
        h.id = h.id + num
        num++
    }
    link.innerHTML =
